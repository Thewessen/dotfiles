let s:containers = 'docker container ls --all --format "%s" | tail +2'
let s:cmd = ''
...
command! -nargs=* -complete=customlist,s:custom_list Docker call s:docker_cmd('', 'Start -title=exec-$1 docker exec -it $1 $2', <f-args>)
command! -nargs=* -complete=customlist,s:custom_list DockerLogs call s:docker_cmd('Logs', 'Start -title=logs-$1 docker container logs -f $1 --tail 1000', <f-args>)
command! -nargs=* -complete=customlist,s:custom_list DockerRestart call s:docker_cmd('Restart', 'Dispatch! docker container restart $1', <f-args>)
command! -nargs=* -complete=customlist,s:custom_list DockerStop call s:docker_cmd('Stop', 'Dispatch! docker container stop $1', <f-args>)
